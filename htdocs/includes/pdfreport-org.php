...
$organizations = new Hmg\Models\Organizations();
...
$recording = $organizations->getList(false);
						...
						case "row":
							foreach($data->data as $row){
								switch ($row->type) {
									...
									break;
									case "field":										
										$pdf->text($row->value, @$row->width, @$row->variableWidth, @$row->wordWrap, @$row->fontSize, @$row->bold, @$row->italic, @$row->fontFamily);										
									break;
									case "image": 
