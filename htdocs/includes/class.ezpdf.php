...
foreach($cols as $k1=>$v1){
  $options['cols'][$k1] = array('width'=> ($this->ez['pageWidth']/count($cols)) );
} 
  ...
  foreach($maxWidth as $colName => $w){
    ...
    if (isset($options['cols'][$colName]) && isset($options['cols'][$colName]['width']) && $options['cols'][$colName]['width']>0 && $w > $options['cols'][$colName]['width']){
