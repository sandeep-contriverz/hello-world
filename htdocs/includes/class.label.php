...
require_once(dirname(__FILE__) . '/class.ezpdf.php');
...
var $ourname="United Way";
var $image=array('width'=>72, 'height'=>0, 'loc'=>"./images/msboa_logo.jpg"); //image used on nametag
var $font_dir="/var/www/html/s4s.unitedwayuc.org/htdocs/newsubforsanta/office/reports/classes/fonts/"; // location of fonts - be sure to include the slash at the end
var $header_text=array(1=>'United Way',2=>'Sub for Santa',3=>'Mailing lists'); // header text - used as three lines
...
function setLabelType($type) {
	switch ($type) {
		case "Av5160": // standard three column page of mailing labels
			...
			break;
		case "Av5962": // standard three column page of mailing labels
			// Avery 5160
			// these labels are page size: letter and portrait orientation
			// create a new page referring to parent class
			$this->size="letter";
			...
			// top and left margins are set to writing point (FYI: exact label edge is 1/2 inch = 36)
			// bottom and right are adjusted to allow the write point to move freely (this is touchy)
			$this->margin_t=$this->u2px(7/8); // 2/3 of an inch
			$this->margin_b=$this->u2px(7/8);
			$this->margin_l=$this->u2px(5/16);
			$this->margin_r=$this->u2px(5/16);
			// set the labels width & height
			$this->label['width']=$this->u2px(4); //width of label = 2 3/4 inches
			$this->label['height']=$this->u2px(1 + (5/16)); //height of label = 1 inch
			// set number of lables per row
			$this->label['per_row']=2;
			// set size of text for label
			$this->label['textsize']=8;
			break;
