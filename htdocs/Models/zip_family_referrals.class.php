...
class ZipFamilyReferrals
{
    ...
    public function __construct($zipCodes = array())
    {
        $this->_zipCodes = $zipCodes;
    }

    public function zipCodesMysql()
    {
        $zipCodesRange = '';
        if (is_array($this->_zipCodes)) {
            foreach ($this->_zipCodes as $county) {
                $zipCodesRange .= ($zipCodesRange ? ', ' : '') . '"' . mysql_real_escape_string($county) . '"';
            }
        }
    ...
    private function buildQuery($addLimit = false)
    {
        ...
        $filter_by = '';
        $filter_by_date = '';
        $join_clause = ' LEFT JOIN families ON family_referrals.family_id = families.id';
        $join_selects = '';
        $having_clause = '';
        ...
        $sql = 'SELECT
                    ...
                    GROUP_CONCAT(DISTINCT(families.zip) ORDER BY families.zip SEPARATOR ", ") zipcodes,
                    count(*) totals '
                . $join_selects
                ...
                . $join_clause
                . ' WHERE families.zip IN (' . $this->zipCodesMysql() . ') '
                . $group_clause
                . $order_by;
        if ($addLimit) {
