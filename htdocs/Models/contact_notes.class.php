...
class ContactNotes
{
    private $_table = 'contact_notes';
    private $_sorts = array('modified' => 'DESC');
    private $_start = 0;
    private $_limit = 0;
    private $_fkey = 'contact_id';
    ...
    public function getList()
    {
        ...
        $sql = 'SELECT `' . mysql_real_escape_string($this->_table) . '`.*, DATE_FORMAT(`' . mysql_real_escape_string($this->_table) . '`.modified, "%m/%d/%Y") formatted_date, users.hmg_worker FROM `' . mysql_real_escape_string($this->_table) . '`
