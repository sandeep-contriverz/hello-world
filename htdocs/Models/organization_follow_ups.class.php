...
class OrganizationFollowUps
{
    ...
    private function buildQuery($addLimit = false)
    {
        ...
        $sql = 'SELECT `' . mysql_real_escape_string($this->_table) . '`.*'
                ...
                . ', DATE_FORMAT(follow_up_date, "%m/%d/%y") follow_up_date_formatted,
                    referred_to.name referred_to, referred_to_id '
                . $join_selects
                . ', referred_to.disabled referred_to_disabled'
                . ', service.disabled service_disabled,referred_to.name organization_name,sn.name site_name, users.hmg_worker'
                ...
                . $join_clause
                . ' WHERE organization_sites_id = "' . mysql_real_escape_string($this->_organization_id) . '" '
                . $order_by;
        if ($addLimit) {
    ...
    public function displayEnumSelect($name, $field, $selected, $label = '', $tabIndex = '', $style = '')
    {
        ...
        if ($rs) {
            ...
            $select = '<select id="' . $field . '" class="' . $field . '" name="' . $name . '" tabindex="' . $tabIndex . '" style="' . $style . '">' . $options . '</select>';
            return $select;
        }
    }
