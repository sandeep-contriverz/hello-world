...
{
    private $_table = 'family_follow_up';
    ...
    private function buildQuery($addLimit = false)
    {
		...
		$sql = 'SELECT `' . mysql_real_escape_string($this->_table) . '`.*'
				...
				. ' LEFT JOIN organization_sites os 
					...
					LEFT JOIN `users` u ON family_follow_up.hmg_worker = u.id'
                . $join_clause
	...
	public function organizationNameAndSite($referred_to_id){
			//get referral name from organization
			$sql = 'Select * from organization_sites os 
				JOIN organizations o ON o.id=os.organization_id
				Where os.id="'.$referred_to_id.'"';
			$rs = mysql_query($sql);
			$row = mysql_fetch_array($rs, MYSQL_ASSOC);
			if(!empty($row)) {
				$referral = !empty($row['organization_site_id'])
