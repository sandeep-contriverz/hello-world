...
class PdfReferralsZip extends FPDF
{
    ...
    function Header()
    {
        ...
        if($this->PageNo() > 1){
            ...
            $this->setX(25);
            // Arial bold 15
            $this->SetFont('CenturyGothic','',16);
            $this->Cell(0,40,'Referrals By Zip Code (cont.)',0,1,'L');

            $this->SetFont('Arial','B',16);
            $this->Cell(114.18,20.87,$this->zip['name'],0,0,'L');
            ...
            //$this->Ln(20);
            // Title
            $this->Cell(0,57.14,'Referrals By Zip Code',0,1,'L');
    ...
    function ZipInformation($info = array()){
        ...
        $this->Cell($this->GetStringWidth('Zip Code: '),20.87,'Zip Code:',0,0,'L');
        $this->SetFont('Arial','',11);
    ...
    function ReferralList($columns = array(), $data = array()){
        ...
        foreach ($data as $row) {
            ...
            if (count($row) == 5) {
                ...
                $zipcodes = $this->NbLines($row['Zip Codes'], 50);
                $count = $this->NbLines($row['Count'], 50);

                $numberLines[] = $referral['lines'];
                $numberLines[] = $zipcodes['lines'];
            ...
            foreach ($row as $column => $value) {
                ...
                } else if ($column == 'Zip Codes') {
                    $cellText = $value;
                    if ($zipcodes['lines'] < $maxLines) {
                        $difference = $maxLines - $zipcodes['lines'];
                        if ($difference) {
                            $cellText .= str_repeat("\n ", $difference);
                        }
                    }
