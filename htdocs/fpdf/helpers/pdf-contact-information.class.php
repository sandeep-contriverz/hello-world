...
class PdfContactInfo extends FPDF
{
    ...
    function Header()
    {
        ...
        if($this->PageNo() > 1){
            ...
            $this->Cell(0,40,'Provider Information (cont.)',0,1,'L');
            ...
            //$this->Ln(20);
            // Title
            $this->Cell(0,57.14,'Provider Information',0,1,'L');
    ...
    function FamilyInformation($info = array()){
        ...
        $this->Cell($this->GetStringWidth('Organization Name: '),20.87,'Organization Name:',0,0,'L');
        $this->SetFont('Arial','',11);
        $this->Cell(114.18,20.87,$info['organizationName'].(!empty($info['site'])?':'.$info['site']:''),0,0,'L');
        $this->Ln(20);
        
        $this->setX(34.28);
        $this->SetFont('Arial','B',9);
        $this->Cell($this->GetStringWidth('Last Name: '),20.87,'Last Name:',0,0,'L');
        $this->SetFont('Arial','',9);
        $this->Cell(285.71,20.87,$info['last'],0,0,'L');
        $this->Ln(20);
		
		
		$this->setX(34.28);
        $this->SetFont('Arial','B',9);
        $this->Cell($this->GetStringWidth('First Name: '),20.87,'First Name:',0,0,'L');
        $this->SetFont('Arial','',9);
        $this->Cell(285.71,20.87,$info['first'],0,0,'L');
        $this->Ln(20);
        ...
        $this->setX(34.28);
        $this->SetFont('Arial','B',9);
        $this->Cell($this->GetStringWidth('Service Area: '),20.87,'Service Area:',0,0,'L');
        $this->SetFont('Arial','',9);
        $this->Cell(114.18,20.87,$info['service_area'],0,0,'L');
        $this->Ln(20);
    ...
    function EventInformationHeader($columns, $cWidth, $cont = false){
        ...
        $this->setX(28.57);
        $this->SetFont('Arial','B',10);
        $headline = 'Event Information ' . ($cont ? ' (cont.)' : '');
    ...
    function EventInformation($columns = array(), $data = array()){

        $cWidth = array(87.14, 57.14, 57.14, 57.14, 67.14, 217.14);

        $this->EventInformationHeader($columns, $cWidth);
        ...
        foreach($data as $row){
            ...
            $currentX = $this->getX();
            $cIndex   = 0;
            $notesLength = $this->GetStringWidth($row['Notes']);
            $numberLines = 1;
            if($notesLength > 217.14){
                $numberLines = ceil($notesLength / 217.14);
                $cellBorder  = "LR";
            ...
            if($pageY > 770){
                ...
                $this->EventInformationHeader($columns, $cWidth, true);
            ...
            foreach($row as $column => $value){
                ...
                $cellText = $value . ($column != 'Notes' ? $cellLinePadding : '');
    ...
    function ResourceInformation($columns = array(), $data = array()){
        ...
        // Display Table
        $cWidth = array(114.28, 114.28, 57.14, 87.14, 170);
        ...
        foreach($data as $row){
            ...
            $referral = $this->NbLines($row['Referral Name'], 114.28);
            $service = $this->NbLines($row['Service'], 110);
            $outcome = $this->NbLines($row['Outcome'], 57.14);
            $date = $this->NbLines($row['Ref. Date'], 87.14);
            $notes = $this->NbLines($row['Notes'], 170);
