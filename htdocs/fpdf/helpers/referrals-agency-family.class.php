...
class PdfReferralsAgencyFamily extends FPDF
{
    ...
    function Header()
    {
        ...
        if($this->PageNo() > 1){
            ...
            $this->Cell(0,40,'Families Referred to an Agency (cont.)',0,1,'L');
            ...
            //$this->Ln(20);
            // Title
            $this->Cell(0,57.14,'Families Referred to an Agency',0,1,'L');
    ...
    function AgencyInformation($info = array()){
        ...
        $columnY = $this->getY();
        $this->SetFont('Arial','B',12);
        $this->Cell($this->GetStringWidth('Families Referred To: '),20.87,'Families Referred To:',0,0,'L');
        $this->SetFont('Arial','',11);
...
/**
    ...
    */
    function displayTableHeaders(){
        ...
        $cWidth = array(40, 100, 60, 60, 130, 80);
        $cIndex = 0;
    ...
    function ReferralList($columns = array(), $data = array()){
        ...
        foreach ($data as $records) {
            foreach ($records as $row) {
                ...
                $familyStatus = $this->NbLines($row['Family Status'], 40);
                $parentName = $this->NbLines($row['Parent Name'], 100);
                $city = $this->NbLines($row['City'], 60);
                $phone = $this->NbLines($row['Phone'], 60);
                $service = $this->NbLines($row['Service'], 130);
                $outcome = $this->NbLines($row['Outcome'], 80);

                $numberLines[] = $familyStatus['lines'];
                $numberLines[] = $parentName['lines'];
                $numberLines[] = $city['lines'];
                $numberLines[] = $phone['lines'];
                ...
                foreach ($row as $column => $value) {
                    ...
                    if ($column == 'Family Status') {
                        $cellText = $value;
                        if ($familyStatus['lines'] < $maxLines) {
                            $difference = $maxLines - $familyStatus['lines'];
                            if ($difference) {
                                $cellText .= str_repeat("\n ", $difference);
                            }
                        }
                    } else if ($column == 'Parent Name') {
                        $cellText = $value;
                        if ($parentName['lines'] < $maxLines) {
                            $difference = $maxLines - $parentName['lines'];
                            if ($difference) {
                                $cellText .= str_repeat("\n ", $difference);
                            }
                        }
                        ...
                        $cellText = $value;
                        if ($city['lines'] < $maxLines) {
                            $difference = $maxLines - $city['lines'];
                            if ($difference) {
                                $cellText .= str_repeat("\n ", $difference);
                            }
                        }
                        ...
                        $cellText = $value;
                        if ($phone['lines'] < $maxLines) {
                            $difference = $maxLines - $phone['lines'];
                            if ($difference) {
                                $cellText .= str_repeat("\n ", $difference);
                            }
                        }
        ...
        // Totals
        $this->MultiCell(470, 20, 'Totals: ' . $totalRecords, '3', 'R', 0);
    }
